...
class IntensityProxCalibrator(object):
    ...
    def __init__(self):
        ...
        self.sub_input_i = rospy.Subscriber(
            '~input/intensity', ProximityArray, self._intensity_cb)
    ...
    def _intensity_cb(self, msg):
        self.i_last_tm = msg.header.stamp
        self.i_raw = np.array([p.proximity for p in msg.proximities])
        ...
        for dist, diff, const, init in zip(distance, self.i_diff_from_init,
                                           self.i_prop_const, self.i_init):
    ...
    def _tof_cb(self, msg):
        ...
        for i, data_st in enumerate(msg.array):
            if abs((data_st.header.stamp - self.i_last_tm).to_sec()) > \
